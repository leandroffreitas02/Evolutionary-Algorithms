...
class DE:
    ...
    def unmodified_DE(self, fun = 1, N = 50, dim = 2, F = 0.9, Cr = 0.7, max_evaluations = 200, lb = -20, ub = 20, decrease = False): # DE with no modifications
        ...
        for eval in range(max_evaluations):
            ...
            if decrease and eval % 3 == 0 and N > 8:
                idx = np.random.permutation(N)[:1]
                new_population = np.delete(new_population, idx, axis=1)
                new_fitness = np.delete(new_fitness, idx)
                N -= 1
    ...
    def DE_F(self, fun = 1, N = 50, dim = 2, max_evaluations = 200, lb = -20, ub = 20, 
               F_max = 0.9, F_min = 0.5, Cr = 0.7, randomize = False, prob = 0.05, dist = 0.01, 
               random_lb = 0.001, random_ub = 1):
        ...
        for eval in range(max_evaluations):
            ...
            new_population, new_fitness, F_data, Cr_data = self.pop_gen(fun, population, population_fitness, F_min, F_max, Cr, Cr, randomize,
                                                                         prob, random_lb, random_ub)
            ...
            for i in range(N):
                self.history.append(self.best_fit)
            self.F_history.append((F_min + F_max)/2)
    ...
    def DE_Cr(self, fun = 1, N = 50, dim = 2, max_evaluations = 200, lb = -20, ub = 20, 
        ...
        for eval in range(max_evaluations):
            ...
            if len(improv_data) > 0:
                ...
                Cr_min = max(0.0001, Cr_mean - 0.1)
                Cr_max = min(1, Cr_mean + 0.1)


            for i in range(N):
                self.history.append(self.best_fit)
            self.Cr_history.append((Cr_min + Cr_max)/2)
    ...
    def pop_gen(self, fun, population, population_fitness, F_min, F_max, Cr_min, Cr_max, randomize = False, prob = 0.05, lb = 0.001, ub = 1):
